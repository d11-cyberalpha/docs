...
```json5
{
  "type": "marketState",
  "data": {
    "m": "us",          // 市场(参考附录:数据字典-市场)
    "s": 0,             // 市场状态(参考附录:数据字典-市场状态)
    "d": 0,             // 交易日类型(参考附录:数据字典-交易日类型)
...
```json5
{
  ...
  "data": {
    ...
    "p": 65535.00,      // 委托价格
    "s": 0.12,          // 委托数量
    "S": "BUY",         // 买卖方向(参考附录:数据字典-买卖方向)
    "y": "LIMIT",       // 交易类型(参考附录:数据字典-交易类型)
    "x": "NEW",         // 订单状态(参考附录:数据字典-订单状态)
    "R": "FREE",        // 风险类型(参考附录:数据字典-风险类型)
    "N": 0.000123,      // 网络手续费(稳定币支付)
    "V": 0.01,          // 网络手续费(原生代币支付)
    "f": "GTD",         // 有效期限(参考附录:数据字典-有效期限)
    "d": 7,             // 有效期
    "st": "DEFAULT",    // 交易时段(参考附录:数据字典-交易时段)
    "T": "USDC",        // 支付代币
    "m": 234.23,        // 订单金额
...
```json5
{
  ...
  "data": {
    ...
    "si": 1,                      // 股票编号
    "y": "LIMIT",                 // 交易类型(参考附录:数据字典-交易类型)
    "x": "CANCELED",              // 订单状态(参考附录:数据字典-订单状态)
    "R": "FREE",                  // 风险类型(参考附录:数据字典-风险类型)
    "r": 1,                       // 原因(参考附录:数据字典-订单失败/取消原因)
    "t": 1725844149,              // 更新时间(Unix时间戳: 秒)
    "E": 1725844149000            // Event time(Unix时间戳: 毫秒)
  }
}
...
```json5
{
  ...
  "data": {
    ...
    "id": 12345678,               // 订单编号
    "si": 1,                      // 股票编号
    "x": "PENDING_CANCEL",        // 订单状态(参考附录:数据字典-订单状态)
    "t": 1725844149,              // 更新时间(Unix时间戳: 秒)
    "E": 1725844149000            // Event time(Unix时间戳: 毫秒)
  }
}
...
| AFTER_HOURS                 | 2 | 盘后    |
| OVERNIGHT                   | 3 | 夜盘    |
| PRE_MARKET_AND_AFTER_HOURS  | 4 | 盘前盘后  |
...
| US_AFTER_HOURS    | 3 | 盘后  |
| CLOSED            | 4 | 已收盘 |
| US_NIGHT_SESSION  | 5 | 夜盘  |

## 10. 交易日类型

| 编码                       | 值 | 说明    |
|--------------------------|---|-------|
| DAY_TYPE_FULL            | 0 | 全天交易  |
| DAY_TYPE_HALF_MORNING    | 1 | 午前半天  |
| DAY_TYPE_HALF_AFTERNOON  | 2 | 午后半天  |